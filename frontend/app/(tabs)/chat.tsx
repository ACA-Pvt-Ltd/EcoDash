...
export default function UserChatScreen() {
  const { collectorName, requestId } = useLocalSearchParams<{ collectorName: string; requestId: string }>();
  const { user } = useAuth();
  ...
  // Must match the room ID used on the collector side
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          <View style={styles.avatar}>
            <Text style={styles.avatarText}>{(collectorName || 'C')[0].toUpperCase()}</Text>
          </View>
          <View>
